...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`sys-locale`](https://crates.io/crates/sys-locale).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `sys-locale` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `sys-locale` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
