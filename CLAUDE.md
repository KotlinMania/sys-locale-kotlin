# Claude Code Project Instructions — sys-locale-kotlin

## Project Overview

This is **sys-locale-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`sys-locale`](https://crates.io/crates/sys-locale).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/sys-locale/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/sys-locale>
