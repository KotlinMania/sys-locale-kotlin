// port-lint: source src/unix.rs (mod tests)
...
private const val BCP_47 = "fr-FR"
private const val POSIX = "fr_FR"
private const val POSIX_ENC = "fr_FR.UTF-8"
private const val POSIX_MOD = "fr_FR@euro"
...
class UnixTests {
    ...
    fun envGet() {
        fun case(
            ...
            env.put(LANGUAGE, language)
            env.put(LC_ALL, lcAll)
            env.put(LC_MESSAGES, lcMessages)
            env.put(LANG, lang)
            val actual = innerGet(env).asSequence().toList()
            assertEquals(expected, actual)
        }

        val env = MockEnv()
        assertNull(innerGet(env).asSequence().firstOrNull())

        // Empty
        case(env, "", "", "", "", emptyList())

        // Constants
        case(env, POSIX_ENC_MOD, POSIX_ENC, POSIX_MOD, POSIX, listOf(BCP_47))
        ...
        case(env, "en_US", "en_US", "ru_RU", "en_US", listOf("en-US", "ru-RU"))
        case(env, "en_US", "ru_RU", "ru_RU", "en_US", listOf("en-US", "ru-RU"))
        case(env, "en_US", "es_ES", "ru_RU", "en_US", listOf("en-US", "es-ES", "ru-RU"))
        case(
            env,
