...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `SysLocale.swiftmodule/`
...
//      `import SysLocale` above would fail at compile time.
//
//   2. The static archive `libSysLocale.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "SysLocale" }` and
//      `flattenPackage = "io.github.kotlinmania.syslocale"` configuration in
...
//      via the `SysLocaleLibrary` product.
//
...
final class SysLocaleExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "SysLocale swift module imported cleanly")
    }
}
